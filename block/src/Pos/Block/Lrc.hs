...
import           Pos.Core (Coin, EpochIndex, EpochOrSlot (..), ProtocolConstants (..), SharedSeed,
                           SlotCount, StakeholderId, crucialSlot, epochIndexL, getEpochOrSlot,
...
lrcDo pm pc epoch consumers = do
  ...
  where
    ...
    withBlocksRolledBack blunds =
        bracket_ (rollbackBlocksUnsafe pm (pcBlkSecurityParam pc) bsc scb blunds)
