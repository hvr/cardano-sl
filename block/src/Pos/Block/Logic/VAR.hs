...
import           Pos.Core (Block, BlockCount, HeaderHash, ProtocolConstants, epochIndexL,
...
verifyAndApplyBlocks pm pc rollback blocks = runExceptT $ do
  ...
  where
    ...
    failWithRollback e toRollback = do
        ...
        lift $ mapM_ (rollbackBlocks pm $ pcBlkSecurityParam pc) toRollback
        throwError e
...
rollbackBlocks pm k blunds = do
    ...
    rollbackBlocksUnsafe pm k (BypassSecurityCheck False) (ShouldCallBListener True) blunds
