...
import           Pos.Core (Block, HasHeaderHash (..), HeaderHash, ProtocolConstants, SlotCount,
                           difficultyL, isMoreDifficult, pcBlkSecurityParam, pcEpochSlots)
...
retrievalWorker pm pc diffusion = do
  ...
  where
    ...
    handleRecovery nodeId rHeader = do
        ...
        void $ getProcessBlocks pm pc diffusion nodeId (headerHash rHeader) checkpoints
