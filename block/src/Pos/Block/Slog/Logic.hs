...
import           Pos.Core (BlockCount, BlockVersion (..), FlatSlotId, ProtocolConstants (..),
                           difficultyL, epochIndexL, flattenSlotId, headerHash, headerHashG,
                           kEpochSlots, pcEpochSlots, prevBlockL)
...
slogRollbackBlocks ::
    ...
    -> m SomeBatchOp
slogRollbackBlocks k (BypassSecurityCheck bypassSecurity) (ShouldCallBListener callBListener) blunds = do
    ...
    let
        secure =
            ...
            maxSeenDifficulty - resultingDifficulty <= fromIntegral k
