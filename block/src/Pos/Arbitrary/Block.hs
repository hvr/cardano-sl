...
instance (HasProtocolConstants, HasProtocolMagic) => Arbitrary (T.BlockHeader 'AttrNone) where
...
instance Buildable (T.BlockHeader 'AttrNone) => Buildable (T.BlockHeader 'AttrNone, PublicKey) where
    build (block, key) =
        bprint
            ( build%"\n"%
              build%"\n"
...
instance Buildable (T.BlockHeader 'AttrNone) => Show BlockHeaderList where
